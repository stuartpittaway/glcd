...
void  loop(){   // run over and over again
  ...
  while(iter++ < 10){   // do 10 iterations
    ...
    //GLCD.FillCircle(centerX/2, centerY-1, centerY/4);   // filled circle in center
...
void scrollingDemo()
{
  ...
  GLCD.FillCircle(GLCD.Width/2, GLCD.Height/2, GLCD.Height/2-1);
  delay(1500);

  for(x = 0; x < 4; x++)
  {
  	GLCD.FillCircle(GLCD.Height/4+GLCD.Height/2*x, GLCD.Height/2, GLCD.Height/4-1, WHITE);
  	delay(500);
  }
